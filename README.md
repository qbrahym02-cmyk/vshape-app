...
| الميزة | التفصيل |
|---|---|
| **رسوم التمارين** 🎨 | رسم توضيحي للوضعية لكل تمرين من الـ٢٠، مع الأداة (حقيبة/طاولة/كرسي/إطار باب/قوارير ماء). مرسوم بـ `CustomPaint` في `lib/widgets/exercise_art.dart` → **صفر بايت زيادة في الـ APK**، ويتلوّن مع الثيم. مصغّر في البطاقة وكبير داخل «طريقة التنفيذ». أي تمرين جديد يُضاف في `content.json` بدون رسم → يظهر بوضعية احتياطية (ما ينكسر شيء). |
| **عدّاد سعرات وبروتين دقيق** 🧮 | كل صنف في `food.meals[].items[]` صار له `protein_g` و`kcal` (المجاميع مطابقة تماماً لهدف الوجبة — يختبره CI). العدّ صار دقيقاً بدل التقسيم التقريبي، والملفات القديمة تشتغل كما كانت. |
| **إضافات سريعة** ➕ | `food.extras` في `content.json`: ١٢ صنف خارج الخطة بأزرار +/− تُحسب فوراً في بروتين وسعرات اليوم. |
| **رسم ٧ أيام** 📈 | أعمدة بروتين وسعرات مع خط الهدف والمتوسط (`WeekBars` في `lib/widgets/painters.dart`). |
| **ودجت الشاشة الرئيسية** 🧩 | ماء/بروتين/سعرات/سلسلة/تمرين اليوم. Android `RemoteViews` فقط (بدون Compose أو مكتبات إضافية). الأرقام **والنصوص المترجمة** تُرسل من Dart (`WidgetData`)، ولقطة محفوظة تُسترجع بعد إعادة التشغيل. |
| **حماية CI** 🛡️ | البناء يفشل لو اختلف `content.json` عن النسخة المدمجة. |

**المحتوى صار v4** — يعدّل `content.json` ويرفع `"version"` فيصل للجوالات بدون تحديث التطبيق.
الميزات الثلاث نفسها تحتاج **تحديث التطبيق** (شريط «فيه نسخة جديدة»).
