...
1. عدّل الكود وارفع `version:` في `pubspec.yaml` — مثال: `1.1.0+2`
   (الرقم بعد `+` هو **versionCode** وهو اللي يقارنه التطبيق).
2. `git commit -am "v1.1.0" && git tag v1.1.0+2 && git push origin main --tags`
3. GitHub Actions يبني APK موقّع وينشره في **Releases** تلقائياً (يشغّل الاختبارات أولاً).
